...
CS 581
Homework 4
To compile: mpic++ lifeMPI.cpp -O -o lifeMPI -std=c++11
To run: mpirun -n <numProcesses> ./lifeMPI <numRows> <numCols> <maxGens> <numProcesses> <fileDirectory>
*/
...
void printMatrix(const vector<vector<cell>> &matrix, int numRows, int numCols, bool clearScreen = false)
{
    if (clearScreen)
    {
#ifdef _WIN32
...
bool processSection(int startRow, int endRow, const vector<vector<cell>> &matrix, vector<vector<cell>> &newMatrix, int numRows, int numCols, int rank)
{
    ...
    // cout << "Rank: " << rank << " Start Row: " << startRow << " End Row: " << endRow << endl;

    for (int i = startRow; i < endRow; i++)
    {
        for (int j = 1; j < numCols - 1; j++)
        { // Skip ghost columns at the boundaries
...
void writeToFile(const vector<vector<cell>> &matrix, int numRows, int numCols, int maxIterations, int numProcesses, const string &fileDirectory)
{
    ...
    fileNameStream << "output." << numRows - 2 << "." << maxIterations << "." << numProcesses << ".mpi.txt";
...
int main(int argc, char **argv)
{
    ...
    for (int gen = 0; gen < maxGens; gen++)
    {
        ...
        // cout << "Rank: " << rank << " Local Change: " << localChange << endl;

        // Wait until all processes complete their computation for the current generation
        // MPI_Barrier(MPI_COMM_WORLD);

        // Exchange ghost cell borders with neighbors using MPI_Sendrecv
        if (rank > 0)
        {
            // Send to upper neighbor and receive from them
            MPI_Sendrecv(&newMatrix[startRow][0], numCols, MPI_CELL, rank - 1, 0,
                         &newMatrix[startRow - 1][0], numCols, MPI_CELL, rank - 1, 1,
                         MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        }
        if (rank < size - 1)
        {
            // Send to lower neighbor and receive from them
            MPI_Sendrecv(&newMatrix[endRow - 1][0], numCols, MPI_CELL, rank + 1, 1,
                         &newMatrix[endRow][0], numCols, MPI_CELL, rank + 1, 0,
                         MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        }

        // Print the matrix at this generation
        //printMatrix(newMatrix, numRows, numCols);

        // Wait until all processes complete the ghost cell exchange
        // MPI_Barrier(MPI_COMM_WORLD);
