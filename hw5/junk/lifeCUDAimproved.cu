...
// This program implements the Game of Life using CUDA with shared memory.
// This is the improved version of the program that uses a 16x16 thread grid per block. The paper claimed that a 16x16 thread grid produced the best performance.
//
