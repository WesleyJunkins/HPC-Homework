...
__global__ void processSection(cell *matrix, cell *newMatrix, int numRows, int numCols)
{
    ...
    if (i >= 0 && i < numRows && j >= 0 && j < numCols) // Ensure inside bounds
    {
        // printf("Value at matrix index: %d, row: %d, column: %d     -     %d\n", index, i, j, matrix[i * numCols + j].getStatus()); // Debugging thread index

        bool isGhost = matrix[i * numCols + j].getGhostStatus();

        if (isGhost)
        {
            newMatrix[i * numCols + j].setGhost();
        }
        else
        {
            // int numNeighbors = getNumNeighbors(matrix, numRows, numCols, i, j);
            int numNeighbors = 0;
            numNeighbors += matrix[(i - 1) * numCols + j].getStatus();
            numNeighbors += matrix[(i + 1) * numCols + j].getStatus();
            numNeighbors += matrix[i * numCols + (j - 1)].getStatus();
            numNeighbors += matrix[i * numCols + (j + 1)].getStatus();
            numNeighbors += matrix[(i - 1) * numCols + (j - 1)].getStatus();
            numNeighbors += matrix[(i - 1) * numCols + (j + 1)].getStatus();
            numNeighbors += matrix[(i + 1) * numCols + (j - 1)].getStatus();
            numNeighbors += matrix[(i + 1) * numCols + (j + 1)].getStatus();
